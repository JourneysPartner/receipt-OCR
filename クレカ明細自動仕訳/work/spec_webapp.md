...
**計測段階は遅い。使い始める前に受入 12 を済ませること。**`WEBAPP_MAX_PER_CALL_` が 1 なので、n 件の押下は n 回の呼出しに分かれる。**1 回の往復は `106 + 2m`**（m ＝ その呼出しで送った `decisions` の数）── 内訳は入口の `authorize` 2 ＋ 1 件の実費 74 ＋ 後始末 34 ＋ 事前走査 2m。押下全体は `Σ_k (106 + 2(n−k+1)) × 単価`：
...
**受入 12 を済ませるまで、この画面を常用してはならない。**2 件の押下でも 5.8 分 ＝ **1 件 174 秒**で、§1.1 が改善対象にした「1 件 40 秒」の **4.4 倍**である。計測段階は受入 12 のための測定そのものと、動作確認だけに使う。
...
> **その帰結を、§12.3 のケース文が隠してはならない。**ハーネスは `src/*.gs` しか読み込まないので、`81_WebAppUi.html` の分岐は**どれ 1 つ壊しても 993 が緑のまま**である（2026-09-16 のレビューが実測：クライアントの無限ループ防止を 2 箇所削っても緑）。**クライアント側の規定を固定していると読めるケース文を書かないこと** ── ケース 28・35・36 は「サーバーが正しいクライアントを書くのに足る情報を返すこと」を固定しているのであって、出荷されるクライアントが正しいことは固定していない。**クライアントの正しさを担保するのは §13 の実機受入だけである。**
