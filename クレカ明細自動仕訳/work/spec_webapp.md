...
| K-W19 | **取込が 6 分に当たった後、取引が誰にも動かせない状態で残る。**2026-09-16 の実機：`202502.xlsx` が `WRITING` のまま、取引 32 件のうち `COMMITTED` 29・**`REVIEW_REQUIRED` 3**。要確認は 0 件なので、**その 3 件を解決する手段が無い** | 複製の帳簿に確定しない行が 3 行残る。K-W11（6 分に当たる）と K-W12（戻す手段が画面に無い）が合わさった実害であり、**`opsRecoverStuckFiles` は K-W10 で使えない**ので ops からも戻せない | 第2段。K-W10 の差し替え口が入れば `opsRecoverStuckFiles` が使えるようになる。**それまでは、取込が 6 分に当たったファイルは手で確認するしかない** |
