...
class VisionOcrEngine(OcrEngine):
    ...
    def extract_text(self, file: DriveFile) -> OcrResult:
        try:
            ...
            logger.error(f"OCR失敗: {file.file_name}: {e}",
    ...
    def _from_image(self, file: DriveFile) -> OcrResult:
        ...
        logger.info(f"画像OCR: {file.file_name} ({len(anns[0].description)} chars)",
                     extra={"step": "ocr", "file_id": file.file_id})
    ...
    def _from_pdf(self, file: DriveFile) -> OcrResult:
        ...
        logger.info(f"PDF OCR: {file.file_name} ({pages} pages)",
                     extra={"step": "ocr", "file_id": file.file_id})
        return OcrResult(raw_text=combined, engine="vision", confidence=avg,
