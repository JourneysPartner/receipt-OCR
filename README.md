...
| D  | 3     | フォルダURL | レシート保存先 Drive フォルダ |
| F  | 5     | 状態 | 処理結果（自動更新、下記参照） |
| G  | 6     | シートリンク | 出納帳URL（空なら自動作成） |
| H  | 7     | 種別 | 個人/法人（テンプレート選択に使用） |
...
1. マスターシートを読み込み
2. 記帳区分=「当方記帳」の行だけ処理
3. G列が空 → H列の種別に応じてテンプレートから出納帳を自動作成 → G列に書き戻し
...
| 種別 | テンプレート |
|------|------------|
| 個人 | `INDIVIDUAL_TEMPLATE_SPREADSHEET_ID` |
| 法人 | `CORPORATE_TEMPLATE_SPREADSHEET_ID` |

- Drive API の `files.copy` でテンプレートを複製
- ファイル名: `【顧客名】現金出納帳`
- 保存先: `CASHBOOK_OUTPUT_FOLDER_ID`
- 作成後、URL を マスター G列 に書き戻し
- **記帳対象タブ名を `【顧客名】現金出納帳` に rename**（テンプレ内の `現金出納帳` タブから自動変換）

## 記帳対象タブ名の解決ルール

出納帳スプレッドシート内で、実際に記帳する対象のシートタブ名は以下の順で解決します:

1. **第一候補**: `【顧客名】現金出納帳`（正仕様）
2. **第二候補**: `現金出納帳`（旧互換。見つかれば自動で `【顧客名】現金出納帳` に rename）
3. どちらも無ければ第一候補名で処理継続（rename 前のテンプレ初期状態を想定）

顧客処理の冒頭で `ensure_cashbook_tab_renamed()` が呼ばれ、
旧仕様のタブ名は自動で正仕様の名前に rename されます。
...
| `INDIVIDUAL_TEMPLATE_SPREADSHEET_ID` | Yes | | 個人テンプレートID |
| `CORPORATE_TEMPLATE_SPREADSHEET_ID` | Yes | | 法人テンプレートID |
| `CASHBOOK_OUTPUT_FOLDER_ID` | Yes | | 出納帳保存先フォルダID |
