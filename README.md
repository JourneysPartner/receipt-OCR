...
### AI 抽出 retry（0件 / parse_error 時）

初回 AI 抽出が **0 明細** または **JSON パース失敗** だった場合、
Gemini multimodal で画像/PDFから再抽出を試みます（amount_validation NG retry とは**別系統**）。

```
Vision OCR → 初回 AI抽出
              ├─ items 取れた → amount_validation へ進む
              └─ 0件 / parse_error
                  ↓ ENABLE_EXTRACTION_RETRY=true
                  Gemini 抽出 retry（画像/PDF から直接、ファイル内1回）
                      ├─ items 取れた → amount_validation へ進む
                      └─ なお 0件   → 抽出失敗 manual_entry（※AI抽出要確認 等）
```

抽出 retry が成功して amount_validation も走った場合、
**そこで amount NG だったら更に既存の amount_validation retry が動く**こともあります。
ただし呼び出されるのは依然として「ファイルあたり 1 回まで」で、
抽出 retry を実行済みであれば amount_validation retry は同じ extract_from_file を**呼ばずに**再判定します
（同一ファイルへの API 連打防止）。

実際の流れ:
- `Vision → 初回0件 → 抽出retry成功 → amount OK → success`
- `Vision → 初回1件で amount NG → amount retry成功 → success`
- `Vision → 初回0件 → 抽出retry成功 → amount NG → amount retry → ...`

要手入力ラベル:
- 抽出失敗（OCR が空） → `※OCR/抽出要確認`
- 抽出失敗（retry も0件 / parse_error） → `※AI抽出要確認`
- それ以外（API例外など） → `※抽出失敗要確認`
- 金額NGによる manual_entry → `※金額要確認`（既存）

### 金額検証 NG 時の Gemini 再抽出

金額検証 (`amount_validation`) が `digit_inflation` または `missing_in_ocr` で NG になったとき、
**そのまま manual_entry にせず、Gemini multimodal に画像/PDFを直接渡して再抽出**します。

```
Vision OCR → AI抽出 → amount_validation
                          ├─ OK   → 通常書き込み
                          └─ NG   → Gemini 再抽出（画像/PDF 直接）
                                       └─ 再 amount_validation
                                            ├─ OK → Gemini 結果で書き込み
                                            └─ NG → manual_entry（候補は維持）
```

**ポイント**:
- 再抽出は **ファイルあたり 1 回のみ**（NG 明細が複数あっても同一 retry 結果から取り出す）
- primary が OK の明細は触らない（safety first）
- `ENABLE_AMOUNT_VALIDATION_RETRY=false` で無効化可能（既定 `true`）
- `OCR_FALLBACK_ENGINE` の confidence-based fallback とは独立（別軸の対策として共存）

**AI詳細ログの memo**:
- `retry=no | amount_status=ok | adopted=primary` （正常）
- `retry=gemini_image | amount_status=digit_inflation→ok | adopted=retry` （再抽出採用）
- `retry=gemini_image | amount_status=missing_in_ocr→missing_in_ocr | adopted=primary` （再抽出却下、manual_entry 行き）
...
| `ENABLE_EXTRACTION_RETRY` | | true | AI 抽出 0件 / parse_error 時の Gemini 再抽出 |
| `ENABLE_AMOUNT_VALIDATION_RETRY` | | true | amount_validation NG 時の Gemini 再抽出 |
