...
既に記帳済みのレシートを再処理しないための印として、人手で `[済]` プレフィックスを付けることを想定しています。
除外プレフィックスは `EXCLUDED_FILE_NAME_PREFIXES` 環境変数で変更可能。

### 勘定科目コード変換

`CASHBOOK_ACCOUNT_CODE_MAP` に JSON でマッピングを設定すると、AI が抽出した勘定科目名を C列のコードに変換して書き込みます。未登録の勘定科目では C列を触りません。

```bash
CASHBOOK_ACCOUNT_CODE_MAP='{"消耗品費": "401", "旅費交通費": "412", "車両費": "425"}'
```
...
| `CASHBOOK_ACCOUNT_CODE_MAP` | | `{}` | 勘定科目名→コード変換表（JSON） |
